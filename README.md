...
<details>
<summary><b>11. PR 템플릿 체크리스트 항목의 미완료 작업 표시 문제</b></summary>

- **상황**: PR 템플릿에서 선택적 체크리스트 항목(`README 업데이트 (필요한 경우)`)을 사용하지 않아도 미완료 작업으로 PR에 표시됨.
- **문제**: 
  - GitHub는 체크박스(`- [ ]`)가 체크되지 않으면 PR 상단에 미완료 작업으로 표시
  - 선택적 항목이라 사용하지 않는 경우에도 계속 미완료로 남아있음
  - PR 목록에서도 완료되지 않은 작업으로 표시되어 혼란 유발
- **해결 방법**:
  - 선택적 항목을 HTML 주석(`<!-- -->`)으로 처리하여 PR에 표시되지 않도록 함
  - 필요할 경우 주석을 해제하여 체크박스로 사용할 수 있도록 안내 추가
  - 예: `<!-- 필요할 경우 주석 해제: - [ ] README 업데이트 -->`
- **추가 개선**:
  - PR 템플릿을 카테고리별로 분리(`feat`, `fix`, `docs` 등)
  - `.github/pull_request_template/` 디렉토리에 각 카테고리별 템플릿 파일 생성
  - GitHub에서 PR 생성 시 템플릿 선택 드롭다운으로 필요한 템플릿만 사용
- **교훈**: 
  - 선택적 항목은 주석 처리하여 불필요한 미완료 작업 표시 방지
  - 카테고리별 템플릿 분리로 각 PR 타입에 맞는 체크리스트만 표시
  - PR 템플릿은 프로젝트의 워크플로우와 일치하도록 지속적으로 개선 필요
</details>
